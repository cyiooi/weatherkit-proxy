...
export default class AirQuality {
    ...
    static Pollutants2AQI(airQuality, Settings, options = {}) {
        ...
        switch (algorithm) {
            ...
            default: {
                ...
                // UBA 的 LQI 是 0-4 的小数分值，但 FlatBuffer 的 index 字段是 Int16 只能存整数，
                // 且 iOS 要求 index 与 categoryIndex 一致才能渲染卡片。与上游 NSRingo/WeatherKit
                // 一致：先按通用算法算出 categoryIndex，再令整数 index = categoryIndex（1-5），
                // 二者恒等，避免 index/categoryIndex 错位导致客户端隐藏 UBA 卡片。
                // （此前用 floor(primaryPollutant.index) 会得到 index=3 而 categoryIndex=4，二者错位。）
            ...
            case "iRingo":
                switch (Settings?.AirQuality?.Calculate?.Algorithm) {
                    ...
                    case "WAQI_InstantCast_CN_25_DRAFT":
                        ...
                        break;
                    case "CN_DEATH_AQHI":
                        scaleDesc = " (China CDC Death Risk AQHI)";
                        break;
                    case "CA_AQHI":
                        scaleDesc = " (InstantCast with CA AQHI)";
                        break;
                    case "HK_AQHI":
                        scaleDesc = " (InstantCast with HK AQHI)";
                        break;
                    case "CN_DEATH_HK_AQHI":
                        scaleDesc = " (China CDC Death Risk + HK AQHI)";
                        break;
                    case "AQHI_Multi_CN":
                        scaleDesc = " (AQHI-Multi for CN)";
                        break;
                    case "AQHI_Multi_CN_HK":
                        scaleDesc = " (AQHI-Multi for CN+HK)";
                        break;
                    case "UBA":
                    default:
                        scaleDesc = " (FB001846)";
    ...
    static #PollutantsToAQHIMulti(pollutants, scale, options = {}) {
        ...
        const usePM10 = contributions.PM10 > 0 && (contributions.PM2_5 <= 0 || contributions.PM10 > contributions.PM2_5 * 1.2);
