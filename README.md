...
- ⚙️ **多预设参数配置 (Preset Panels)**：
  ...
  - **数据源深度定制**：在高级配置中，可独立选择并混合搭配天气、降水、天气预警、空气指数、污染物等不同提供商。
  - **天气预警补全**：默认解析 Apple FlatBuffer 提供的和风灾害预警页面，不需要和风 API Key。也可显式选择自有和风 API（需控制台专属 `*.qweatherapi.com` Host 与 Key）或彩云 CAP API，补全 Apple 已有摘要。第三方不可用时仍保留 Apple 原始预警，不新增第三方预警。
